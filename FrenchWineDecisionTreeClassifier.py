...
plt.figure(figsize=(20, 10)) #利用seaborn库绘制三种法国红酒不同参数图
...
(X_train, X_test, Y_train, Y_test) = train_test_split(all_inputs, all_species, train_size=0.85, random_state=1)#85%的数据选为训练集
...
# 使用训练的模型进行预测，为了方便，
# 案例直接把测试集里面的数据拿出来三条
print(X_test[0:5])#利用5个数据进行测试，即取3个数据作为模型的输入端
model.predict(X_test[0:5])
frenchwine=[]
for X in model.predict(X_test[0:5]):
     if X=="Zinfandel":
        frenchwine.append("仙粉黛")
     elif X=="Sauvignon":
          frenchwine.append("赤霞珠")
     else:
          frenchwine.append("西拉")
print(frenchwine)
